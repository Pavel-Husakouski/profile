...
- [ ] Прогнать результат через любой резюме-парсер: совпали ли имя, титул, компании, даты.
      13.09.2026 при первом прогоне парсер не увидел пунктов списка — маркеры рисовались как
      CSS `::marker` (PDF) и через `numbering.xml` (docx), то есть мимо текста. Теперь маркер
      пишется буквальным `- ` в обоих форматах: 49 пунктов из 49 видны при извлечении.
