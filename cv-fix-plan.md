...
- [x] **Даты в формате `Oct 2023 – Present`.** Сделано 13.09.2026: все десять дат переведены из `10.2023` в `MMM YYYY`, `present` стал `Present`. В `make-pdf.py` регулярки `DATE_LINE` и разбор хвоста заголовка теперь принимают оба формата, даты по-прежнему уходят вправо в строку заголовка.
...
## P5. Гигиена репозитория — закрыт

- [x] **Разбор резюме в репозитории.** Репозиторий приватный, поэтому `Pavel Husakouski — Resume Audit.md`, `Recruiter feedback.md` и этот план остаются в нём осознанно.
- [x] **Каталог не путается с резюме.** Переименован в `catalogue.generated.md` и собирается из `cv.md` через `make-catalogue.py`; имя говорит, что ручные правки в нём будут затёрты.

Единственное, что вернёт этот раздел к жизни: если репозиторий когда-нибудь станет публичным, вместе с резюме станут читаемыми и три документа о его слабых местах. Тогда либо вынести их, либо оставить приватным.

---

## Ближайшие шаги, по отдаче

Порядок на случай, если возвращаешься к работе через неделю и не помнишь, где остановился.

1. **Секция `Skills` вместо `Experience with:`, плюс `Professional Summary`** — P0.3, P1.2, P1.3, P4.1. Одной правкой закрывается пункт рекрутёра и дефект парсинга: сейчас инвентарь технологий для ATS вообще не привязан к разделу. Текст готов, твоих данных не требует.
2. **Даты в формате `Oct 2023 — Present`** — P4.1. Механически, но затрагивает `DATE_LINE` и разбор хвоста заголовка в `make-pdf.py`; после правки проверить, что даты по-прежнему уходят вправо в PDF.
3. **Строка про AI с событием вместо ярлыка** — P0.5. Требует твоих фактов: какими агентами работаешь, что это дало измеримо, подхватила ли команда.
4. **Решить даты Alpari** — P0.6. `10.2023 – present` при двух закрытых проектах из трёх; читатель не понимает, работаешь ты сейчас или нет.

Дальше по остаточному принципу: возврат потерянных фактов (внутренний DSL в VPI, `single-handedly` и фреймворк в OData-пункте, скрипты в рунбуке), результаты по education assistant и платформам Exadel, уровень 3 сокращений, если понадобится ужаться сильнее.
